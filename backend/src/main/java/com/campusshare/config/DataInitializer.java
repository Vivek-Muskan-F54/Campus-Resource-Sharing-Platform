...
public class DataInitializer {
 @Bean CommandLineRunner seed(UserRepository users,CategoryRepository categories,PasswordEncoder encoder,@Value("${app.admin.email}")String email,@Value("${app.admin.password}")String password){return args->{
  if(!users.existsByEmail(email)){User u=new User();u.setName("Platform Admin");u.setEmail(email);u.setPassword(encoder.encode(password));u.setCollegeRollNumber("ADMIN");u.setRoles(Set.of(Enums.Role.ADMIN));u.setEmailVerified(true);u.setVerificationStatus(VerificationStatus.APPROVED);users.save(u);}
